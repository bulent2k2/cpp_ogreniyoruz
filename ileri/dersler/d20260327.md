...
Bir de sıra kalıbının tür girdisi `Nokta` yerine `Hat`, yani bir çift kullandık:
```c++
using Hat = std::pair<U, Şehir>;
```
Bu hat türünü aslında çizgenin bağlarını belleğe yazmak için kullanacağız. Öncelik sırası içindeki anlamı biraz farklı: Sıradaki her öge, ikinci türün belirlediği şehre şu ana kadar bulduğumuz en kısa yolun uzunluğunu tutacak. Bu çift kalıbının içindeki tür girdilerinin sırası çok önemli! Çünkü öncelik sırası soldan başlayarak karşılaştırma yapıyor. Aynı şehre farklı yollardan varmak da mümkün elbet. Biz hep en yakın olanına öncelik vermek istiyoruz.  
...
Daha detaylı bir örneği okuyup kurcalamak isterseniz, şuna bakıverin: [İşlevci Tür ve Nesneleri](https://onlinegdb.com/4AeWb8U3qu).  
