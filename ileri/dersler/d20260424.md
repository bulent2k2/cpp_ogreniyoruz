...
Adres değişkenleri, iki boyutlu diziler, vb..
====
...
İlk boyutun uzunluğunu derleyici, eşittir işaretinin sağındaki sabit dizi değerine bakarak bulabiliyor. İkinci ve varsa sonraki boyutların uzunluğunu biz belirliyoruz. 
...
Farketmişsinizdir, iki döngü birbirine çok benziyor. Sadeleştirme iyi olur:
...
Şimdi biraz daha genelleyelim. Boyut uzunlukları değişken olsun:
```c++
    K b1 = 4, b2 = 7;
    S** d;
    d = new S*[b1];
...
Tabii `yaz()` işlevi pek anlamlı birşey yazmayacak. Ufak bir değişiklik yapalım:
```c++
void yaz(auto d, K b1, K b2) { 
    S s = 11;
    for (K k1 = 0; k1<b1; ++k1) {
...
Tabii bellekte epey yer ayırdığımız için, temizliğini de yapmamız gerekir. O da kolay:
```c++
    for (K k=0; k<b1; ++k)
        delete [] d[k];
    delete [] d;
