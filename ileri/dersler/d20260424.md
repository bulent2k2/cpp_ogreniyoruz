...
---
Bir zarı bir çok kere atarak bir dizi elde ediyoruz. Yani dizimizin sayıları hep 1, 2, 3, 4, 5 ya da 6. 

Bir de bize bir hedef toplam veriliyor: `n`.

Kaç değişik zar dizisi bu toplamı verir acaba? 

Aradığımız bu çözüm sayısını matematiksel bir fonksiyon olarak `f(n)` diye yazalım. Bunu daha küçük değerlerini kullanarak nasıl yazarız? Önce toplamı sıfır olan tek dizimiz var, boş dizi, onun için: 
```math 
f(0) = 1
```
olsun. Örneğin, toplamı üç olan dizilerin sayısını, toplamı iki, bir ve sıfır olan dizilerin sayılarını kullanarak bulalım:
```math
f(3) = f(0) + f(1) + f(2)
``` 
Değil mi? Neden? Çünkü:
Boş diziden sonra zar 3 gelmeli, `{1}` dizisinden sonra 2 gelmeli ve, {1,1} veya {2} dizisinden sonra da 1 gelmeli. 
```math
f(1) = 1
``` ve
```math
