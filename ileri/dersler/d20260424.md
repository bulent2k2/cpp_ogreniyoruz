...
Bu kodu [buradan](https://onlinegdb.com/HAN8BByDp) *Fork this* ile çatallayın. *Debug* tuşuna basın. `main()` satırının solundaki satır sayısına tıklayarak bir *breakpoint* yani durak koyun. Sonra *start*. Sonra da *step into* tuşuyla satır satır çalıştırın. Böylece adresleri bütün detaylarıyla görmek faydalı olabilir.

İşlevin girdisi, adres yerine sayının değeri olsaydı, sayıyı değil, kopyasını değiştirirdi. Bu arada kopya yerine benzer ya da ikiz de diyebiliriz, ne dersiniz? Örneğin:
```c++
using S = int;
